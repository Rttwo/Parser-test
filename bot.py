...
# ── DEX цены (DexScreener) ────────────────────────────────────
async def get_dex_prices(tokens: list[dict]) -> dict[str, float]:
    """Берём цену с DexScreener — пул с максимальным объёмом торгов"""
    prices = {}
        ...
        for chain, mints in by_chain.items():
            for i in range(0, len(mints), 30):
                ...
                for pair in pairs:
                    ...
                    if quote not in ("USDC", "USDT", "WETH", "WBNB", "WSOL"):
                        continue
                    ...
                    if addr and price_str:
                        try:
                            ...
                            if addr not in best or liq > best[addr][1]:
                                best[addr] = (price, liq)
...
async def get_dex_info(mint: str, chain: str) -> dict | None:
    """Получаем полную инфу по токену с DexScreener для проверки при добавлении"""
    async with httpx.AsyncClient(timeout=15) as client:
        url = f"https://api.dexscreener.com/tokens/v1/{chain}/{mint}"
        try:
            r = await client.get(url)
            r.raise_for_status()
            pairs = r.json()
    ...
    async with httpx.AsyncClient(timeout=10) as client:
        try:
            ...
            for item in data:
                ...
                if sym.endswith("_USDT") and price:
                    try:
                        all_prices[sym.replace("_USDT", "")] = float(price)
                    except (ValueError, TypeError):
                        pass
            for symbol in symbols:
                if symbol in all_prices:
                    ...
                    logger.warning(f"MEXC Futures: {symbol}_USDT не найден")
        except Exception as e:
...
async def main():
    # Создаём приложение с обработчиком команд
    ...
    await verify_tokens_on_start(bot)

    # Запускаем цикл цен и polling параллельно
