            ...
            for i in range(0, len(mints), 30):
                ...
                for item in items:
                    ...
                    if not addr or not price_str:
                        logger.warning(f"GeckoTerminal {chain}: пропуск {symbol_gt} addr={addr} price={price_str}")
                        continue
                    # Все адреса в lowercase
                    addr = addr.lower()
                    try:
                        prices[addr] = float(price_str)
                        logger.info(f"GeckoTerminal {chain}: {symbol_gt} = ${float(price_str):.8f}")
...
async def build_status_text() -> str:
    ...
    if now_ts - _price_cache["ts"] < CACHE_TTL and _price_cache["dex"]:
        dex_prices  = _price_cache["dex"]
        mexc_prices = _price_cache["mexc"]
    else:
        dex_prices  = await get_dex_prices(TOKENS)
        mexc_prices = await get_mexc_prices([t["symbol"] for t in TOKENS])
        _price_cache = {"dex": dex_prices, "mexc": mexc_prices, "ts": now_ts}
