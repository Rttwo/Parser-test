...
# GeckoTerminal обновляется быстрее DexScreener (~5-10 сек vs 30-60 сек)
...
async def get_dex_prices(tokens: list[dict]) -> dict[str, float]:
    """Берём цену с GeckoTerminal — быстрее DexScreener (~5-10 сек задержка)"""
    prices = {}
                    ...
                    for mint in chunk:
                        ...
                        try:
                            ...
                            if price_str:
                                ...
                                logger.info(f"TON {mint[:10]}: ${float(price_str):.8f}")
                        except Exception as e:
                            logger.error(f"GeckoTerminal TON {mint[:10]}: {e}")
                    continue
