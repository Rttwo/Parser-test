...
async def get_dex_prices(tokens: list[dict]) -> dict[str, float]:
    """Берём цену с DexScreener — пул с максимальной ликвидностью.
    Используем /latest/dex/tokens/ для Solana и /tokens/v1/ для EVM.
    """
    prices = {}
    ...
    async with httpx.AsyncClient(timeout=15) as client:
        for chain, mints in by_chain.items():
            for i in range(0, len(mints), 30):
                chunk = mints[i:i+30]
                # Solana использует старый endpoint который работает лучше
                if chain in ("solana", "ton"):
                ...
                try:
                    ...
                    r.raise_for_status()
                    data = r.json()
                    # /latest/dex/tokens/ возвращает {"pairs": [...]}
                    # /tokens/v1/ возвращает [...]
                ...
                for pair in pairs:
                    ...
                    if quote not in ("USDC", "USDT", "WETH", "WBNB", "WSOL", "TON"):
                        continue
