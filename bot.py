...
async def build_status_text() -> str:
    ...
    # DEBUG: логируем ключи Solana цен
    sol_keys = [k for k in dex_prices if len(k) > 30 and not k.startswith("0x")]
    logger.info(f"STATUS: Solana ключей в dex_prices: {len(sol_keys)}, примеры: {sol_keys[:2]}")

    ok_lines, problems = [], []
    for t in TOKENS:
        symbol = t["symbol"]
        chain_t = t.get("chain", "solana")
        lookup = t["mint"] if chain_t == "solana" else t["mint"].lower()
        has_dex  = lookup in dex_prices
        has_mexc = symbol in mexc_prices
        if chain_t == "solana" and not has_dex:
            logger.warning(f"STATUS: {symbol} mint={t['mint'][:16]} не найден в dex_prices")
